...
function FlyingBook({ src, title, startX, startY, endX, endY, delay, onLanded }) {
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true; xAnim.cancel(); };
  }, []);

  return (
    <div
      ref={outerRef}
      style={{ position: "fixed", left: startX, top: startY, width: 54, height: 76, zIndex: 1000, pointerEvents: "none" }}
    >
      <div ref={innerRef} style={{ width: 54, height: 76 }}>
        {src ? (
          <img
            src={src}
            alt={title}
            style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 4, boxShadow: "0 4px 16px rgba(0,0,0,0.35)", display: "block" }}
          />
        ) : (
          <div style={{ width: "100%", height: "100%", background: spineColor(title || "x"), borderRadius: 4, boxShadow: "0 4px 16px rgba(0,0,0,0.35)" }} />
        )}
      </div>
    </div>
  );
...
function SideBookcase({ entries, label, emptyMsg, onBookEnter, onBookLeave, onBookClick, isSelected, bookcaseRef, receivedEntries = [] }) {
  const shelves = createShelves(entries, BOOKS_PER_SHELF, MIN_SHELVES);
  const receivedForLastShelf = receivedEntries.slice(0, BOOKS_PER_SHELF - shelves[shelves.length - 1].length);
          ...
          return (
            <div className="side-row" key={shelfIdx}>
              <div className="side-interior">
                {shelfIdx === 0 && entries.length === 0 && receivedEntries.length === 0 && (
                ...
                {shelfBooks.map((entry) => {
                  ...
                  return (
                    <div
                      key={entry.id}
                      className={`side-book${isSelected(entry) ? " side-book--selected" : ""}`}
  ...
  return (
    <div className="shelf-wrapper">
      ...
      <div className="shelf" ref={shelfRef} style={flying ? { visibility: "hidden" } : {}}>
        {offer.map((entry, i) => (
          ...
          >
            <BookCover
              ...
              placeholderClassName="trade-cover-placeholder"
              titleClassName="trade-cover-title"
            />
        ...
        {offer.length === 0 && <span className="empty-hint">NO BOOKS SELECTED</span>}
      </div>
    </div>
  );
}
...
export default function TradePage() {
  ...
  const { theirUserId, theirName: initTheirName, prefilledGoogleId, prefillAll } = location.state || {};

  const { session } = useAuthSession();
  const { hoveredEntry, popupRect, onEnter, onLeave, onPopupEnter, onPopupLeave } = useHoverPopup();
  ...
  function handlePropose() {
    ...
    if (total === 0 || !meetupLocation.trim() || !meetupDateTime) return;
    ...
    function onOneLanded() {
      landedRef.current++;
      if (landedRef.current < total) return;
  ...
  const popupStyle = computePopupStyle(popupRect, POPUP_W, POPUP_H, POPUP_GAP);
  ...
  return (
    <div className="trade-page">
      ...
      {flyingBooks.map((b) => <FlyingBook key={b.key} {...b} />)}
      ...
      {hoveredEntry && (
        ...
        >
          {hoveredEntry.book.thumbnail ? (
            <img src={secureImageUrl(hoveredEntry.book.thumbnail)} alt="cover" className="trade-popup-cover" />
          ) : (
